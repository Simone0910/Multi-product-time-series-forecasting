...
df_all = melt[['YEARMONTH','SALES_TOTAL','SECTOR']]
df_all = df_all.sort_values(by=['SECTOR','YEARMONTH'])

# creiamo un dictionary contenente tutti i settori
settori = df_all.groupby(['SECTOR'])
dict_sectors = {name: group for name, group in settori}

# scegliamo un settore, per provarlo su altri settori basta cambiare il nome TELECOM
# con quello di un altro settore
sector = dict_sectors['TELECOM']
sector = sector.groupby('YEARMONTH').sum()
...
'prophet model'
# train - test split
train,test = train_test_split(sector, train_size=0.85, shuffle=False)
...
plt.title('TOTAL SALES MODEL PREDICTION PER SECTOR: TELECOM', fontsize=15)
...
create_features(sector)
create_features(future)

sectors = sector.rename(columns={'date':'ds',
   'SALES_TOTAL':'y'})
...
import matplotlib.dates as mdates
f, ax = plt.subplots(figsize=(25, 10), dpi=500)
plt.title('TOTAL SALES MODEL PREDICTION PER SECTOR: PICCOLO ELETTRODOMESTICO', fontsize=15)

prophet_data = pd.concat([sector, future], axis=0)
plt.plot(sector.index, sector['y'], label='TOTAL SALES')
...
previous_year_sales = sector[sector.index >= '2021-03-01']
